...
export class ServerConnection implements TListenerTarget<TServerMethod> {
  ...
  /**
   * Adds a listener for a specific server event methods.
   * @param methods The event methods that will be handled. The handler will only
   * be called if one of the methods matches what is sent by the server in a web
   * socket message, except for 'open', 'close', and 'error' events, which are
   * called upon their respective web socket events.
